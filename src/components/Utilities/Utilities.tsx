...
export default function Utilities() {
    return (
        <article className="w-full  p-[110px] flex flex-col justify-center items-start gap-16 h-vh">
            ...
            <section className="w-full">
                <h5 className="text-[32px] text-[#F6FAFF] font-medium text-balance leading-[111%] tracking-[1.28px]">Community is everything. Users, followers, members… We recognize they are the blood to our heart. Our values and actions must reflect our appreciation.</h5>
            </section>
                    ...
                    utilities.map(({ number, text }, index) => {
                        return (
                            <div key={index} className="flex flex-row gap-5 justify-center items-center w-[500px] border-t-[1px] border-[#42474E] border-solid pt-[19px] pb-[35px]">
                                <h4 className="text-[32px] font-extrabold uppercase tracking-[1.28px]">{number}</h4>
                                <p className="text-[24px] text-[#F6FAFF)] font-medium tracking-[0.48px] text-start">{text}</p>
                            </div>
                        )
                    })
                }
